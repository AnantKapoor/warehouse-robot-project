...
public class Task {
    ...
    public float calculateReward(Map<Character, Integer> tasks,GridMap map,GridPose startingPose) {
        ...
        Map<Character, Specifications> specs = itemSpecifications2.getItemSpecification();
        int totalDistance=0;
        ...
        while (it.hasNext()) {
            ...
            while (it2.hasNext()) {
                ...
                char item2 = (char) pair2.getKey();
                if (item2 == item) {
                    Specifications itemData = (Specifications) pair2.getValue();
                    double reward = itemData.getReward();
                    pathInfo= finder.FindPath(currentPose,itemData.getCoordinates());
    ...
    /*public int calculateDistance(Iterator items,GridPose startingPose,GridMap map) {
    	...
    	PathInfo pathInfo=null;
    	while(items.hasNext()) {
    		pathInfo= finder.FindPath(currentPose, itemSpecifications.getItemSpecification().get(items.next()).getCoordinates());
    		totalLength+=pathInfo.path.size();
    		currentPose=pathInfo.pose;
    	}
    	return totalLength;
    }*/
